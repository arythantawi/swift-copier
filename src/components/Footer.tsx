...
import { MapPin, Phone, Mail, ArrowUpRight, X, FileText, Shield } from "lucide-react";
...
const Footer = forwardRef<HTMLElement>((_, ref) => {
  ...
  const [legalDialog, setLegalDialog] = useState<{ open: boolean; title: string; content: string }>({ open: false, title: '', content: '' });
  ...
  const openLegalPage = async (slug: string) => {
    const { data } = await supabase
      ...
      .single();
    if (data) {
      setLegalDialog({ open: true, title: data.title, content: data.content });
    }
  };

  // Simple markdown-like renderer for ## and ### headings, lists, bold
  const renderContent = (content: string) => {
    return content.split('\n').map((line, i) => {
      if (line.startsWith('### ')) {
        return <h4 key={i} className="text-base font-display font-semibold text-foreground mt-6 mb-2">{line.replace('### ', '')}</h4>;
      }
      if (line.startsWith('## ')) {
        return <h3 key={i} className="text-lg font-display font-bold text-foreground mt-4 mb-3">{line.replace('## ', '')}</h3>;
      }
      if (line.startsWith('- ')) {
        return (
  ...
  return (
    <>
        ...
        <div className="border-t border-background/8">
          <div className="container px-4 sm:px-6 py-5 flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <div className="flex items-center gap-5 text-xs">
              <button onClick={() => openLegalPage('terms')} className="text-background/40 hover:text-accent transition-colors flex items-center gap-1.5">
                <FileText className="w-3 h-3" />
                Syarat & Ketentuan
              </button>
              <button onClick={() => openLegalPage('privacy')} className="text-background/40 hover:text-accent transition-colors flex items-center gap-1.5">
      ...
      <Dialog open={legalDialog.open} onOpenChange={(open) => setLegalDialog(prev => ({ ...prev, open }))}>
        <DialogContent className="max-w-2xl p-0 overflow-hidden rounded-2xl border-border/50 shadow-2xl">
          {/* Header with gradient */}
          <div className="relative px-6 pt-6 pb-4 bg-gradient-to-br from-primary/5 via-background to-accent/5 border-b border-border/30">
            <DialogHeader>
              <DialogTitle className="font-display text-xl font-bold text-foreground tracking-tight">
                {legalDialog.title}
              </DialogTitle>
              <p className="text-xs text-muted-foreground mt-1">Terakhir diperbarui • 44 Trans Jawa Bali</p>
            </DialogHeader>
          </div>
          {/* Scrollable content */}
          <ScrollArea className="max-h-[60vh] px-6 py-5">
            <div className="space-y-0">
              {renderContent(legalDialog.content)}
            </div>
          </ScrollArea>
          {/* Footer */}
          <div className="px-6 py-3 border-t border-border/30 bg-muted/30 flex items-center justify-between">
            <p className="text-[11px] text-muted-foreground">© {currentYear} 44 Trans Jawa Bali</p>
            <button
              onClick={() => setLegalDialog(prev => ({ ...prev, open: false }))}
