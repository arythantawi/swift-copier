...
import { Mail, Lock, User, Facebook, Loader2 } from "lucide-react";
const GoogleLogo = () => <svg viewBox="0 0 24 24" width="20" height="20">
...
const Auth = () => {
  ...
  return <div className="min-h-screen bg-white flex justify-center items-center flex-col font-sans py-8 px-4">
      {/* Logo */}
      ...
      boxShadow: "0 14px 28px rgba(0,0,0,0.15), 0 10px 10px rgba(0,0,0,0.12)"
        ...
        <div className={`absolute top-0 h-full transition-all duration-600 ease-in-out left-0 w-1/2 ${isRightPanelActive ? "translate-x-full opacity-100 z-[5] animate-show" : "opacity-0 z-[1]"}`}>
          <form onSubmit={handleSignUp} className="bg-card flex items-center justify-center flex-col px-6 md:px-10 lg:px-16 h-full text-center w-full">
            ...
            <div className="relative w-full mt-4">
              ...
              <input type="text" placeholder="Nama Lengkap" value={signUpName} onChange={e => setSignUpName(e.target.value)} className="bg-white border border-border rounded-md px-10 py-3 my-2 w-full text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary" required />
            </div>

            <div className="relative w-full">
              <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
              <input type="email" placeholder="Email" value={signUpEmail} onChange={e => setSignUpEmail(e.target.value)} className="bg-white border border-border rounded-md px-10 py-3 my-2 w-full text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary" required />
            </div>

            <div className="relative w-full">
              <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
              <input type="password" placeholder="Password" value={signUpPassword} onChange={e => setSignUpPassword(e.target.value)} className="bg-white border border-border rounded-md px-10 py-3 my-2 w-full text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary" required minLength={6} />
            </div>
        ...
        <div className={`absolute top-0 h-full transition-all duration-600 ease-in-out left-0 w-1/2 z-[2] ${isRightPanelActive ? "translate-x-full" : ""}`}>
          <form onSubmit={handleSignIn} className="bg-card flex items-center justify-center flex-col px-6 md:px-10 lg:px-16 h-full text-center w-full">
            ...
            <div className="relative w-full mt-4">
              <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
              <input type="email" placeholder="Email" value={signInEmail} onChange={e => setSignInEmail(e.target.value)} className="bg-white border border-border rounded-md px-10 py-3 my-2 w-full text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary" required />
            </div>

            <div className="relative w-full">
              <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
              <input type="password" placeholder="Password" value={signInPassword} onChange={e => setSignInPassword(e.target.value)} className="bg-white border border-border rounded-md px-10 py-3 my-2 w-full text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary" required />
            </div>

            <a href="#" className="text-sm text-muted-foreground hover:text-primary mt-2">
              Lupa password?
            </a>
